"""FastAPI entrypoint for worker-c."""
...
app = FastAPI(title="Worker C", version="0.1.0")


def build_agent() -> BaseAgent:
    prompt_path = Path(os.getenv("PROMPT_PATH", "/app/config/prompt_evaluate.txt"))
    prompt_text = prompt_path.read_text(encoding="utf-8")
    capabilities = [
        cap.strip()
        for cap in json.loads(os.getenv("CAPABILITIES", "[\"evaluate\"]"))
        if cap.strip()
    ]
    public_url = os.getenv("PUBLIC_URL")
    config = AgentConfig(
        agent_id=os.getenv("AGENT_ID", "worker-c"),
        ...
        ag2_profile=os.getenv("AG2_PROFILE", "worker-evaluate"),
